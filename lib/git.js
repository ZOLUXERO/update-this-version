...
function readCommitHistory(sinceTag = "") {
    ...
    if (sinceTag != "") return  execSync(`git log ${sinceTag}..main --oneline --no-abbrev-commit`, { encoding: 'utf-8' }).trim().split('\n');
...
function addAndCommit(message, version = "") {
    let gitCommand = "";
    gitCommand = execSync(`git add ./CHANGELOG.md ./CURRENT_RELEASE.md ./package.json`, { encoding: 'utf-8' });
